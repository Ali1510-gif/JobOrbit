...
//	}
	...
	public static java.util.Set<Integer> getAppliedJobIds(int userId) throws Exception {
		Connection conn = null;
		PreparedStatement ps = null;
		ResultSet rs = null;
		java.util.Set<Integer> appliedIds = new java.util.HashSet<>();
		...
		try {
			...
			while (rs.next()) {
				appliedIds.add(rs.getInt("job_id"));
			}
			return appliedIds;
		} finally {
			if (rs != null) rs.close();
