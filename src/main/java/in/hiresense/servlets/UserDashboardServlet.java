...
public class UserDashboardServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (resumeUploaded) {
                // Extract comprehensive resume data from Affinda API
                String resultJson = logs.get(0).getResultJson();
                resumeData = AffindaAPI.extractResumeData(resultJson);
            }

            List<JobPojo> jobs = JobDAO.getAllJobsForUserDashboard(search, sort, location, experience, packageLpa);

            // ✅ Calculate comprehensive match score if resume exists
            if (resumeUploaded && resumeData != null) {
                for (JobPojo job : jobs) {
                    // Use comprehensive matching algorithm
                    double score = AffindaAPI.calculateComprehensiveMatchScore(job, resumeData);
                    job.setScore(score);
                }
                
                // Sort jobs by match score (highest first)
                jobs.sort((j1, j2) -> Double.compare(j2.getScore(), j1.getScore()));
            }
